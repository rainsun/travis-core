...
describe AnnotationProvider do
  ...
  describe ".authenticate_provider" do
    context "given a valid username and key" do
      it "authenticates the provider" do
        AnnotationProvider.authenticate_provider(provider.api_username, provider.api_key).should eq(provider)
      end
    end

    context "given an invalid username" do
      it "does not authenticate the provider" do
        AnnotationProvider.authenticate_provider("someone-else", provider.api_key).should be_nil
      end
    end

    context "given an invalid key" do
      it "does not authenticate the provider" do
        AnnotationProvider.authenticate_provider(provider.api_username, "some-other-key").should be_nil
      end
    end
    ...
    context "with an encrypted key" do
      it "authenticates the provider" do
        ...
        AnnotationProvider.authenticate_provider(provider.api_username, "a-key").should eq(provider)
      end
    end
  end
  ...
  describe "#annotation_for_job" do
    ...
    context "given a annotation already exists for the job" do
      it "returns the annotation" do
        annotation = Factory(:annotation, annotation_provider: provider)
        provider.annotation_for_job(annotation.job_id).should eq(annotation)
      end
    end
